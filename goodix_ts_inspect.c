...
static int goodix_save_limits(struct goodix_ts_test *ts_test, struct file *fp)
{
				       ...
				       struct file * fp)
	{
		...
		for (i = 0; i < TOTAL_FRAME_NUM; i++) {
			goodix_data_cal(ts_test->rawdata[i].data, len,
					stat_result);
			bytes += sprintf(
				&data[bytes],
			...
			goodix_data_cal(ts_test->accord_arr[i].data, len,
					stat_result);
			bytes += sprintf(
				&data[bytes],
					  ...
					  struct file * fp)
	{
		...
		for (i = 0; i < NOISEDATA_TEST_TIMES; i++) {
			goodix_data_cal(ts_test->noisedata[i].data, len,
					stat_result);
			bytes += sprintf(
				&data[bytes],
				"<DataContent No.=\"%d\" DataCount=\"%d\" Maximum=\"%d\" Minimum=\"%d\" Average=\"%d\">\n",
				    ...
				    struct file * fp)
	{
		...
		if (ts_test->test_result[GTP_SELFNOISE_TEST]) {
			ret = goodix_save_self_data(
				ts_test, fp, ts_test->self_noisedata.data,
				"selfDiffDataRecord",
		...
		for (i = 0; i < MAX_TEST_ITEMS; i++) {
			...
			if (ts_test->test_result[i]) {
				if (ts_test->test_result[i] == GTP_TEST_PASS)
					goodix_strncatint(
						ts_test->test_info, i, "%dP-",
						TS_RAWDATA_RESULT_MAX);
				else
					goodix_strncatint(
						ts_test->test_info, i, "%dF-",
						TS_RAWDATA_RESULT_MAX);
			}
		}
